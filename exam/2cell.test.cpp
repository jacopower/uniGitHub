...
TEST_CASE("Testing CheckInput()")
{
  ...
  SUBCASE("initial_infected is not valid") {
    ...
    double beta = 0.1;
    double gamma = 0.1;
    CHECK_THROWS_AS(CheckInput(T, L, beta, gamma, initial_infected), const std::invalid_argument&);
  }
...
TEST_CASE("Testing HowManyCloseInfected()")
{
  ...
  SUBCASE("Invalid x") {
    ...
    CHECK_THROWS_AS(mondo.HowManyCloseInfected(-1, 2), const std::invalid_argument&);
  }
  SUBCASE("Invalid y") {
    grid_t grid(3, std::vector<Cell>(3, Cell::s));
    World mondo(3, 1, grid);
...
TEST_CASE("Testing day() in limit cases")
{
  SUBCASE("Testing beta=0 and gamma=0") {
    ...
    for (int k = 0; k < T; k++) {
      ...
      if (static_cast<double>(mondo.get_I()) / static_cast<double>(mondo.get_L() * mondo.get_L()) > 0.3)
        {v_start = true ;}
  ...
  SUBCASE("Testing beta=1 and gamma=1") {
    ...
    CHECK(mondo.get_I() == 0);
  // non si controllano rimossi e suscettibili a causa della casualità dell'infezione tuttavia sicuramente il numero degli infetti è 0
  }
