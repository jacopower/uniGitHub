...
class World {
    private:
      ...
      int S, I, R, V, Q , D;  // parameters that define the world in a specific day (Q=1 :quarantine on, =0 :off)
      std::vector<std::vector<Cell>> m_grid{} ; 

    public:
      World(int L, int Infected, std::vector<std::vector<Cell>> initial_grid) ;   // initializes a word on day 0
