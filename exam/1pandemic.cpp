...
std::vector<SIR> Pandemic::infection(double beta, double gamma, int T, bool vax)
  {
...
void CheckInput(double& t,
                ...
                double const& gamma)
  {
    ...
    if (floor(N) != ceil(N)) {
      throw std::invalid_argument("N° of people (N) must be a positive integer") ;    // invalid_argument() 
    }
    if (floor(I) != ceil(I)) {
