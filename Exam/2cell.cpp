...
World::World(int L, int I, std::vector<std::vector<Cell>> initial_grid)  : L_0{L} ,
                                                                                  S{(L * L) - I} ,
                                                                                  I_0{I} ,
                                                                                  R{0} ,
                                                                                  V{0} ,
...
std::vector<std::vector<Cell>> World::get_grid()  { return grid_0 ; }   // using those methods to access private inforation to prevent changing them unintentionally
...
void World::day(double& beta, double const& gamma, bool vax, bool lockdown, bool v_start )    // changes the parameters of a day to create the following one
{
  ...
  for(int i = 0; i < L_0; i++)    // loops rows
  {
    ...
    for(int j = 0; j < L_0; j++)    // loops columns
    {
      ...
      {
        Randomnumber = p(prng) ;
        if(Randomnumber <= (HowManyCloseInfected(i, j)/8 * beta))   // randomly choose if a cell is becoming ill (based on the illness of the surrounding ones)
        {
...
std::vector<Point> position_s(int L, std::vector<std::vector<Cell>> grid)    // contains the posotions of 's' cells
{
  if(L >= 0)
    {
                ...
                double const& gamma)   // checks if user's imputs have valid arguments
{
  ...
  if (floor(I) != ceil(I)) {
    throw std::invalid_argument("N° of infecteds (I) must be an integer") ;
  }
  if(I < 0 || I > L * L){
