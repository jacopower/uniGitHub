                ...
                double const& gamma)    // checks if user's imputs have valid arguments
{
  ...
  if (I > N) {
    throw std::invalid_argument("N° of infecteds (I) must be smaller than N° of people (N)") ;
  }
  if (beta < 0 || beta > 1) {
...
void Intro() {
  std::cout << "\n\n\n"
            ...
            << std::string(27, ' ') << "|\n"
            << "+"<< std::string(108, '-') << "+\n"
            << std::string(17, ' ') << "|  This simulation uses the SIR model to study the course of an epidemic  |\n"
            << "+"<< std::string(108, '-') << "+\n"
            << std::string(17, ' ') << "| If you wish to abort, at any time, please press Ctrl+C on your Keyboard |\n"
            << "+"<< std::string(108, '-') << "+\n\n" ;
}
